    ...
    if "EDIT_FILES" in action_list:
        ...
        if relevant_files != "":
            ...
            for file_name in fileslist:
                ...
                files_to_construct.append((f"migration_source/{file_name}", file_content))
        ...
        for file_name in file_name_list:
            ...
            try:
                ...
                print(
                    f"File not found: {file_name}. Please ensure the file exists and try again. You can resume the debugging process with the `--step test` flag."
                )
                raise typer.Exit()
...
def debug_testfile(error_message,testfile,globals):
    ...
    relevant_files = construct_relevant_files(
        [(f"migration_source/{testfile}", source_file_content)]
    )
